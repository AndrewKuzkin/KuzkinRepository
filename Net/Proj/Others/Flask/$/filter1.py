...
def index():
    if request.method == 'POST':
        ...
        if check_sql_injection(user_input):
            ...
            logging.info(f"SQL Injection detected - IP: {request.remote_addr}, Input: {user_input}")
        ...
        if check_js_injection(user_input):
            ...
            logging.info(f"JS Injection detected - IP: {request.remote_addr}, Input: {user_input}")
...
'''
при обнаружении SQL-инъекции или JS-инъекции будет создаваться запись в лог-файле app.log с указанием времени, типа инъекции, IP-адреса источника и строки ввода.
'''